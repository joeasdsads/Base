...
public:
	...
	{
		return my_sqrt((x - vector.x) * (x - vector.x) + (y - vector.y) * (y - vector.y));
	}

	static float my_sqrt666(float number) {
		long        i;
		float       x2, y;
	...
	{
		return my_sqrt666(powFFFFFFFFFFFFFFFFFFFFFF(b.x - x) + powFFFFFFFFFFFFFFFFFFFFFF(b.y - y));
	}

	inline float Length()
	{
		return my_sqrt666(x * x + y * y);
	}
...
vector2 normalize2(vector2 norm)
{
	if (norm.x < -89)
		norm.x = -89;
...
public:
	...
	inline float UnityDot(Vector lhs, Vector rhs)
	{
	...
	{
		Vector tis(x, y, z);
		float num = Vector(x, y, z).magnitude();
		if (num > 1E-05f)
		{
			tis /= num;
		}
